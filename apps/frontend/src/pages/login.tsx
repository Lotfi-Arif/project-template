...
const LoginStudent = () => {
  ...
  const [signIn] = useLoginUserMutation({
    onCompleted(data?) {
      ...
      console.log("here in the login", data.loginUser.user, cookie);

      if (data.loginUser.user.role === Role.Admin) {
        router.push("/pages/admin");
      }
      if (
        data.loginUser.user.role === Role.Counselor &&
        data.loginUser.user.accountStatus === AccountStatus.Approved
      ) {
      ...
      if (
        data.loginUser.user.role === Role.Student &&
        data.loginUser.user.accountStatus === AccountStatus.Approved
      ) {
            ...
            <div className="row-auto">
              <form>
                ...
                <button
                  ...
                  className="text-white p-2 m-2 items-center disabled:opacity-70 active:translate-y-0.5 active:border-b-transparent transition duration-200 text-center ease-in-out rounded-xl disabled:cursor-not-allowed font-medium focus:outline-none border-b-4 bg-sky-500"
                >
                  Login
                </button>
                <button
